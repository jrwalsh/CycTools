...
public class CustomInterpreter implements Interpretable {
	...
	public ArrayList<AbstractFrameUpdate> tableToFrameUpdates(TableModel tb) {
		...
		for (int rowIndex = 1; rowIndex < tb.getRowCount(); rowIndex++) {
...
//			frameUpdates.add(new SlotUpdate(frameID, "PubmedID", (String) tb.getValueAt(rowIndex, 4)));
//			frameUpdates.add(new SlotUpdate(frameID, "ReferenceID", (String) tb.getValueAt(rowIndex, 5)));
//			frameUpdates.add(new SlotUpdate(frameID, "Source", (String) tb.getValueAt(rowIndex, 6)));
